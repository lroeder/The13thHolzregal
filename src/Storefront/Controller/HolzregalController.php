...
class HolzregalController extends StorefrontController
{
    ...
    /**
     * @Route("/konfigurator", name="frontend.the13thholzregal.holzregal", methods={"GET","POST"})
     */
    ...
    public function showPage(Request $request, SalesChannelContext $context): Response
    {
        ...
        if(isset($_GET['stuetzart'])) {
            ...
            return $this->renderStorefront('@the13thholzregal/storefront/page/holzregal/index.html.twig', [
                'konfig_id' => $_SESSION['holzregal']['id'],
                'session_id' => session_id()
            ]);    
        }
    ...
    // dDJZSko5NVF3Nm1ERHE3VTBIa3Y1NXR4ZE5vdkJHblhUbWs0RU8
    ...
    private function getRegalAufbau() {
        ...
        $svg = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="'.(($XXBreite + 4.5)*$vFactor +$vAbstandLinks).'" height="'.(370*$vFactor).'">';
        $svg .= '<line x1="'.(0 + $vAbstandLinks).'" y1="'.((5 + (359 - $yyHoehe) - 50)).'" x2="'.($XXBreite*$vFactor + 4.5*$vFactor + $vAbstandLinks).'" y2="'.((5 + (359 - $yyHoehe) - 50)).'" style="stroke:#ababab;stroke-width:1;" />';
        $svg .= '<line x1="'.(0 + $vAbstandLinks).'" y1="'.((359 - $yyHoehe) - 5 - 50).'" x2="'.(0 + $vAbstandLinks).'" y2="'.(25 + (359 - $yyHoehe) - 50).'" style="stroke:#a5c614;stroke-width:2" />';
    ...
    private function getArtikel() {
        ...
        if ($err) {
          ...
          if(date ("Ymd", filemtime($filename)) != date ("Ymd")) {
